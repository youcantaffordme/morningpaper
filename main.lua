-- Morning Paper v0.12.1 — lead-first newsroom + robust delivery cadence
--
-- Keep the proven v0.7 build pipeline, but return a real subclass here instead
-- of relying on a side-effect scheduler wrapper. This makes the cadence menu
-- override unambiguous to KOReader's plugin loader.
...
-- Expand the research/agenda pool before the base plugin initializes.
...
-- Install the consensus paywall-lead editorial layer before main_v07 captures
-- the Newsroom module reference.
require("newsroom_v12")
local Base = require("main_v07")

local MorningPaper = Base:extend{
    name = "morningpaper",
    is_doc_only = false,
}
...
function MorningPaper:init()
    ...
    -- Preserve the base wake handler's boolean safety check while making the
    -- cadence setting the real source of truth.
...
function MorningPaper:autoStatusText()
    ...
    local delivery = os.date("%I:%M %p", os.time{
        ...
        hour=self.auto_hour or 6, min=self.auto_minute or 30, sec=0,
    }):gsub("^0", "")
...
function MorningPaper:addToMainMenu(menu_items)
    Base.addToMainMenu(self, menu_items)
    ...
    local items = menu_items.morning_paper.sub_item_table
    items[3] = {
    ...
    if items[6] then
        items[6] = {
            text=_("About"),
            callback=function()
                UIManager:show(InfoMessage:new{ text=_("Morning Paper 0.12.1 uses a consensus lead-first AI newsroom. Major public paywall headlines act as importance signals, accessible reporting supplies the evidence, and automatic delivery supports every morning, weekdays, weekly, or manual-only schedules.") })
            end,
        }
    end
end
